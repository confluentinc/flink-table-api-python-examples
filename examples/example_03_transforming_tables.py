...
def run():
    ...
    transformedOrders = filteredOrders.add_columns(
        col("price").round(0).cast(DataTypes.INT()).alias("price_rounded")
    ...
    joinedTable = customers.join(
        transformedOrders, col("customer_id") == col("o_customer_id")
